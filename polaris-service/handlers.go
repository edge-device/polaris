...
// App.deleteToken() deletes existing refersh token
func (a *App) deleteToken(w http.ResponseWriter, r *http.Request) {
	...
	if !ok {
		log.Println("Error asserting pathVarsKey context to string.")
		http.Error(w, "getting endpoint context", http.StatusBadRequest)
		return
	}
	tid := pathVars["tokenID"]

	// Parse access token for token ID
	...
	if err != nil {
		log.Println("App.deleteToken(): token validation failed: ", err)
		writeJSONResponse(w, http.StatusBadRequest, nil)
		return
	}
	claims, ok := token.Claims.(jwt.MapClaims)
	if !ok || !token.Valid {
		log.Println("App.deleteToken(): token claims validation failed: ")
		writeJSONResponse(w, http.StatusBadRequest, nil)
		return
	}

	// Delete all refresh tokens for TID
	tokenID := strconv.FormatFloat(claims["tid"].(float64), 'f', 0, 64)
	if tokenID != tid {
		log.Println("App.deleteToken() ID in token didn't match path. ", err)
		writeJSONResponse(w, http.StatusUnauthorized, nil)
		return
	}

	log.Println("Delete tokeID: ", tokenID) // TODO: remvoe this debug message
	query := `
		DELETE 
		FROM access_token 
		WHERE token_id = ?;`
	...
	if err != nil {
		log.Println("App.deleteToken() problem executing delete", err)
		writeJSONResponse(w, http.StatusBadRequest, nil)
		return
	}
